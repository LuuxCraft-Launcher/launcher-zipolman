...
class Splash {
    ...
    async dowloadUpdate() {
        ...
        const githubAPI = await fetch('https://api.github.com').then(res => res.json()).catch(err => err);

        const githubAPIRepoURL = githubAPI.repository_url.replace("{owner}", repoURL[0]).replace("{repo}", repoURL[1]);
        const githubAPIRepo = await fetch(githubAPIRepoURL).then(res => res.json()).catch(err => err);

        const releases_url = await fetch(githubAPIRepo.releases_url.replace("{/id}", '')).then(res => res.json()).catch(err => err);
